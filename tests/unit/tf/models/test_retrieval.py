...
def test_two_tower_shared_embeddings():
    ...
    item_tower = mm.Encoder(schema.select_by_tag(Tags.ITEM), mm.MLPBlock([256, 128]))
    query_tower = mm.Encoder(schema.select_by_tag(Tags.USER), mm.MLPBlock([256, 128]))

    model = mm.TwoTowerModelV2(
        query_tower=query_tower,
        candidate_tower=item_tower,
